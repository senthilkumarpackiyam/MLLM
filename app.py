...
# Uses your existing Phase-4/5 pipeline: FAISS (text+image), embeddings, reranker, sources.
...
# BRAND / THEME
# =======================
BRAND_BLUE = "#0078FF"      # VERINT blue (adjust if needed)
...
<style>
    ...
    /* Make general markdown text readable deep blue */
    .stMarkdown, .stMarkdown p, .stMarkdown li, .stMarkdown span {{
        color: {DEEP_BLUE} !important;
    }}
    /* Centered VERINT header */
    .verint-header {{
        width: 100%;
        text-align: center;
        font-size: 3rem;
    ...
    /* Sidebar section titles (match Retrieval Settings blue) */
    section[data-testid="stSidebar"] h2 {{
        color: {BRAND_BLUE} !important;
    }}
    /* Download transcript label in same bright blue */
    .download-transcript {{
        color: {BRAND_BLUE};
        font-weight: 700;
        font-size: 1.1rem;
...
# config.json contains your paths (created earlier)
...
TEXT_EMB_NPY    = EMBED_DIR / "text_embeddings.npy"
TEXT_CHUNKS_JSONL = CHUNK_DIR / "text_chunks.jsonl"
...
def load_all():
    ...
    img_index  = faiss.read_index(str(IMAGE_INDEX_PATH)) if IMAGE_INDEX_PATH.exists() else None
    text_meta  = pd.read_csv(TEXT_META_CSV) if TEXT_META_CSV.exists() else pd.DataFrame()
    img_meta   = pd.read_csv(IMG_META_CSV) if IMG_META_CSV.exists() else pd.DataFrame()
    text_emb   = np.load(TEXT_EMB_NPY).astype("float32")

    # chunk_id -> full JSON record (to fetch chunk text)
    chunk_by_id: Dict[str, Dict[str, Any]] = {}
    if TEXT_CHUNKS_JSONL.exists():
...
def retrieve(query: str, k_text=8, k_img=4, mmr_pool=50, lambda_div=0.55, per_doc_cap=2, use_images=True):
    ...
    if use_images and img_index is not None and img_index.ntotal > 0 and not img_meta.empty and k_img > 0:
        ...
        for idx, sc in zip(iI, iD):
            if idx == -1 or idx >= len(img_meta): continue
